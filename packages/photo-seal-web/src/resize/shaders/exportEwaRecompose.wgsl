...
fn sampleResidual(srcPos: vec2<f32>, invSrc: vec2<f32>, lowCenter: vec4<f32>) -> vec4<f32> {
  ...
  let center = samplePremultTex(sourceUv(srcPos, invSrc));
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  // TDT-PHOTOSEAL-13-H3-R4: no residual sharpen on export path.
  // The previous residual recompose could create visible ringing around high-contrast edges.
  let out = safeUnpremult(vec4<f32>(max(lowPremult.rgb, vec3<f32>(0.0)), clamp(lowPremult.a, 0.0, 1.0)));
